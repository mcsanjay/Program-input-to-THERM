...
def main_prog (lines, name = None, out_file_path = None, doc_file_path = None, bd_grp = None):
	...
	print ("\nAlkane gauche = {} \nAlkene gauche = {}".format(gauche_ane,gauche_ene))
	...
	if (rad == 1):
		print("\nNo. of rotors (Parent) = {}".format(rotors[0]))
		print("No. of rotors (Radical) = {}".format(rotors[1]))
	else:
	...
	if (rad == 1):
		print("\nSymmetry No. (Parent) = {}".format(symm_no[0]))
		print("Symmetry No. (Radical) = {}".format(symm_no[1]))
	else:
	...
	if (rad == 1):
		if (bd_grp == None):
			...
			bd = bd_grp
			print ("\nbd = {} (from SMILES file)".format(bd))
		
		#timeout = 5
		if ((out_file_path != None) and (bd_grp == None)):
			therm_config_file = open("./therm.config",'r')
			t_lines = therm_config_file.readlines()
			therm_config_file.close()
			for i in t_lines:
				words = i.split()
				if (len(words) == 0):
					continue
				if (words[0][0] == '#'):
					continue
				if (words[0] == "WAIT_TIME"):
					if ((len(words) < 3) or (words[2][0] == '#')):
						print('\nWarning : Waiting Time not specified in "therm.config" !!!\nTaking default Waiting Time as 5 seconds...\n')
						timeout = 5
					else:
						timeout = int(words[2])
	...
	if (doc_file_path != None):
		create_dotdoc.create_doc(gauche_ane,gauche_ene,h_repel_no,oi,grps,rotors,symm_no,bd,no_of_c,no_of_o,no_of_h,name,rad,doc_file_path)
